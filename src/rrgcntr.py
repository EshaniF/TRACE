...
Two mechanisms replace the original four additions:
  ...
  B. Inference-time re-ranking  [evaluation]
     ...
     additive correction in log-space. Which mechanism supplies that
     correction is controlled by --rerank-mode:
       "transition" (default) — TransitionGraph.rerank_scores(), using
            the loaded LLM/empirical/random/shuffled transition graph
            (see --transition-graph-path).
       "frequency" / "recency" — FrequencyReranker.rerank_scores(), a
            graph-free baseline that ignores relation semantics and
            only looks at recent (subject, object) co-occurrence.
       "none" — re-ranking disabled entirely, regardless of
            --rerank-alpha.
...
class RecurrentRGCN(nn.Module):
    ...
    # -----------------------------------------------------------------------
    # Inference  — base scoring + re-ranking (mode-dispatched)
    # -----------------------------------------------------------------------
    ...
    def predict(self, que_pair, sub_graph, T_id, test_graph,
        ...
        recent_events : list of (s, r, o) int tuples from the most recent
                        ...
                        mainneg.py.  Used only when self.rerank_mode is
