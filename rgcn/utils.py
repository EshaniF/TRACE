...
def sort_and_rank_filter(batch_a, batch_r, score, target, all_ans):
    ...
    #The first column increases in increments, and the second column indicates the position of the corresponding answer entity id in each row
    indices = indices[:, 1].view(-1)
    return indices
...
def split_by_time(data):
    ...
    for i in range(len(data)):
        ...
        #latest_t indicates the time when the last triplet was read. The triples in the data set must be sorted in chronological order.
...
def slide_list(snapshots, k=1):
    ...
    :return:
    """
    k = k  # k=1 需要取长度k的历史，在加1长度的label: We need to take a history of length k and add a label of length 1.
    if k > len(snapshots):
...
def construct_snap(test_triples, num_nodes, num_rels, final_score, topK):
    ...
    # 转化为numpy array: Convert to numpy array
