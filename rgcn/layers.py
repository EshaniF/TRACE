...
class RGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, bias=None, activation=None,
        ...
        if self.skip_connect:
            self.skip_connect_weight = nn.Parameter(torch.Tensor(out_feat, out_feat))   # 和self-loop不一样，是跨层的计算 - Unlike self-loop, it is cross-layer calculation
            nn.init.xavier_uniform_(self.skip_connect_weight,
                                    gain=nn.init.calculate_gain('relu'))

            self.skip_connect_bias = nn.Parameter(torch.Tensor(out_feat))
            nn.init.zeros_(self.skip_connect_bias)  # 初始化设置为0 - Initialized to 0
    ...
    def forward(self, g, prev_h=[]):
        ...
        if len(prev_h) != 0 and self.skip_connect:
            skip_weight = F.sigmoid(torch.mm(prev_h, self.skip_connect_weight) + self.skip_connect_bias)     # 使用sigmoid，让值在0~1 - Use sigmoid to keep the value between 0 and 1
            # print("skip_ weight")
            # print(skip_weight)
        ...
        self.propagate(g)  # 这里是在计算从周围节点传来的信息 - Here we are calculating the information coming from the surrounding nodes.
        ...
        # print(len(prev_h))
        if len(prev_h) != 0 and self.skip_connect:   # 两次计算loop_message的方式不一样，前者激活后再加权 - The two ways of calculating loop_message are different. The former is activated and then weighted.
...
class UnionRGCNLayer(nn.Module):
    ...
    def msg_func(self, edges):
        ...
        #What is counted here is the message sent by each node, which is used as the header entity when the node sends a message
...
class RGAT(nn.Module):
    ...
    def forward_v(self,g,batchsize,node,rel,tim):
        g.ndata['h'] = node
        g.edata['rel_emb'] = rel[g.edata['type']]
        # g.edata['tim_emb'] = tim[g.edata['time']]  # 这里可以保留或者放在外面，同时分block可以放在这里面 This can be kept or placed outside, and the sub-blocks can be placed inside
        if self.self_loop:  # 这里是true : Here is true
            # print(self.loop_weight)
            # loop_message = torch.mm(g.ndata['h'], self.loop_weight)
            loop_message = g.ndata['h']
            if self.dropout is not None:
                loop_message = self.dropout(loop_message)  # 一部分变成0，损失信息: Part of it becomes 0, and information is lost
        # 修改思路：Modification ideas
        # 主要是在消息传播的时候会超出内存，后面不会超出，在传播的前一步g分batch: 
        #The main problem is that the memory will exceed the limit when the message is propagated, but it will not exceed the limit later. In the previous step of propagation, the batch is divided
        # 在信息传播 处做修改 : Make changes in information dissemination
        # 每一个batch计算出一部分嵌入，把所有的按照该有的顺序合成原有的size: Each batch calculates a part of the embedding and synthesizes all of them in the order they should be to the original size

        # sampler =dgl.dataloading.MultiLayerFullNeighborSampler(1)
        # sampler = dgl.dataloading.MultiLayerNeighborSampler([100])
        # train_nids = np.random.choice(np.arange(g.num_nodes()), (g.num_nodes(),), replace=False)#这里是选取输出尾节点 : Here is the output tail node
        # dataloader =dgl.dataloading.NodeDataLoader\
        #     (g, train_nids, sampler, batch_size=batchsize, device=g.device)#一个block的output_nodes的数量是batch_size: The number of output_nodes in a block is batch_size
        ...
        self.propagate(g)  # 这里out of memory//信息传播: Here is out of memory//information propagation
        # node_repr = b
        # if self.self_loop:
        #     node_repr = node_repr + loop_message  # 两个信息相加（原信息和传播之后信息）: Add two pieces of information (original information and information after transmission)
        # g.ndata['h'] = node_repr  # 替换 replace
        # quad_rel = torch.matmul(rel, self.W_r)
    ...
    def forward_v2(self,g,batchsize,node,rel,device):
        ...
        dataloader =dgl.dataloading.DataLoader\
            (g, train_nids, sampler, batch_size=batchsize, device=device)#一个block的output_nodes的数量是batch_size: The number of output_nodes in a block is batch_size
...
class UnionRGATLayer(nn.Module):
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        g.apply_edges(self.edge_attention) #获取注意力机制: Get the attention mechanism
...
class CompGCNLayer(nn.Module):
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        if self.self_loop:
            ...
            # loop_message[masked_index, :] = torch.mm(g.ndata['h'], self.loop_weight)[masked_index, :]
        if len(prev_h) != 0 and self.skip_connect:
            skip_weight = F.sigmoid(torch.mm(prev_h, self.skip_connect_weight) + self.skip_connect_bias)     # 使用sigmoid，让值在0~1: sigmoid keep between 0 and 1
